...
Komponenta interně využívá kaskádové zapojení osmi menších čítačů (`counter2_bcd`). Každý z nich reprezentuje jeden řád času:
...
Testbench (`time_counter_tb`) systematicky prověřuje všechny stavy komponenty v reálném provozu. 
...
*(Obrázek: Průběh signálů ze simulace testbenche ukazující funkční přetečení do následujícího čítače)*